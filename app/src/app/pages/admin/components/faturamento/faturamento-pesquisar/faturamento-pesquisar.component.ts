...
import { Faturamento, FaturamentoGrid } from '../../../models/faturamento.model';
...
@Component({
  selector: 'app-faturamento-pesquisar',
  templateUrl: './faturamento-pesquisar.component.html',
  styleUrls: ['./faturamento-pesquisar.component.css'],
})
export class FaturamentoPesquisarComponent extends FormBaseComponent
  implements OnInit {
  public static ROTA: string = '/admin/faturamento';
  ...
  carregarComboStatus(){
    this.faturamentoService.obterStatus().subscribe(
      resp => {
  ...
  carregarComboTipoFaturamento(){
    this.faturamentoService.obterTipoArquivo().subscribe(
      resp => {
        resp.result.forEach(data => {
  ...
  criarSubheader() {
    ...
    this.subheader.title = this.mensagens['MSG000273.descricao'];
  ...
  carregarPagina() {
    ...
    //   permission: ['faturamento.cadastrar', 'faturamento.atualizar'],
    //   click: (obj: Faturamento) =>
    ...
    //   visible: true,
    //   permission: ['faturamento.cadastrar', 'faturamento.atualizar'],
    //   click: (obj: Faturamento) => this.reProcessar([obj]),
    // };
    ...
    this.activeRoute.params.subscribe((params) => {
      ...
      let id = params['id'];
      if(id!=null){
        gridOption.apiServiceHttp = `${environment.apiUrl}/${DocumentoService.API_DOCUMENTOS}/${id}/faturamentos${window.location.search}`;
      }else{
        gridOption.apiServiceHttp = `${environment.apiUrl}/${FaturamentoService.API_FATURAMENTO}${window.location.search}`;
      }
    });

    // gridOption.routeBase = 'admin/faturamento';
    ...
    gridOption.onSelectedItem = (itens: Faturamento[]) => {
    ...
    gridOption.onBenforeData = (data: Faturamento[]) => {
      var dados: FaturamentoGrid[] = [];
      data.forEach(entity => {

        var dado:FaturamentoGrid = <FaturamentoGrid>entity;
  ...
  ativar(itens: Faturamento[]) {
        var status = itens[0].status == 0 ? 1 : 0;
    var requisisoes = [];
    itens.forEach((item) => {
      requisisoes.push(
        this.faturamentoService.statusFaturamento(item.id, status).pipe(catchError(erro => of(erro)))
      );
    });

    forkJoin(requisisoes).subscribe((x: RetornoPadrao<Faturamento>[]) => {
            var resp = x[0];
  ...
  reProcessar(itens: Faturamento[]) {
    ...
    modalRef.result.then(
      () => {
        ...
        itens.forEach((item) => {
          registros.push(
            this.faturamentoService.deletarFaturamento(item.id).pipe(catchError(erro => of(erro)))
          );
        });

        forkJoin(registros).subscribe((resp: RetornoPadrao<Faturamento>[]) => {
